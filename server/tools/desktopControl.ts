...
async function getNut() {
  if (!nut) {
    ...
    // Keep automation snappy; tune if actions fire too fast for slow apps.
    nut.mouse.config.mouseSpeed = 2500;
...
export async function captureScreen(maxWidth = 1280): Promise<ScreenshotResult> {
  ...
  const pngBuffer = await pipeline.jpeg({ quality: 75 }).toBuffer();
